...
class FichaCliente {
    ...
    public static function getByClienteId(int $cliente_id): ?self {
        ...
        $stmt = $pdo->prepare("SELECT * FROM ficha_cliente WHERE cliente_id = :cliente_id");
    ...
    public static function getById(int $id): ?self {
        $pdo  = Database::getInstance()->getConnection();
        $stmt = $pdo->prepare("SELECT * FROM ficha_cliente WHERE id = :id");
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    private static function mapRow(array $row): self {
        return new self(
            ...
            isset($row['largo_espalda']) ? (float)$row['largo_espalda'] : null,
            isset($row['largo_pantalon']) ? (float)$row['largo_pantalon'] : null,
    ...
    public function guardarOActualizar(): bool {
        ...
        return ($existente !== null) ? $this->actualizar() : $this->guardar();
    }

    private function guardar(): bool {
        $pdo = Database::getInstance()->getConnection();
        $stmt = $pdo->prepare("INSERT INTO ficha_cliente (cliente_id, talle, contorno_pecho, contorno_cintura, contorno_cadera, largo_manga, largo_espalda, largo_pantalon, observaciones_cliente) VALUES (:cliente_id, :talle, :contorno_pecho, :contorno_cintura, :contorno_cadera, :largo_manga, :largo_espalda, :largo_pantalon, :observaciones_cliente)");
    ...
    private function actualizar(): bool {
        $pdo = Database::getInstance()->getConnection();
        $stmt = $pdo->prepare("UPDATE ficha_cliente SET talle = :talle, contorno_pecho = :contorno_pecho, contorno_cintura = :contorno_cintura, contorno_cadera = :contorno_cadera, largo_manga = :largo_manga, largo_espalda = :largo_espalda, largo_pantalon = :largo_pantalon, observaciones_cliente = :observaciones_cliente WHERE cliente_id = :cliente_id");
